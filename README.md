...
当前保留 dwconv 的局部/粗尺度门控网络和目标区域结构损失，并纳入 slmk 的数据增强及 src 工具。传播距离为 0.20 m / 0.10 m，有效采样为 13.0 μm / 6.4 μm。硬件像元仍为 8.0 μm / 4.5 μm。

教师输出目录为 `output/Tv2_dmd640_contextdw_d20d10_p13_v2`，学生目录为 `output/SLM_Tv2_dmd640_contextdw_d20d10_p13_v2`。学生默认读取该新目录下的教师检查点。先训练新教师，再训练学生；旧 `contextdw_d20d10_v1` 和 slmk 的检查点不对应此配置。`direct_sgd` 为直接相位参数化名称，学生优化器仍为 Adam。
