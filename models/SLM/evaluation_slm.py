...
def evaluate_slm_detector(
    ...
    totals = {key: 0.0 for key in (
        ...
        "roi_structure", "roi_gradient", "roi_pyramid", "roi_correlation",
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader, desc="SLM validation", leave=False, disable=not is_main):
            ...
            if evaluate_detector:
                predictions = detector_core(
                    prepare_slm_detector_feature(config, student_feature)
                )
            ...
            for key in ("roi_structure", "roi_gradient", "roi_pyramid", "roi_correlation"):
                totals[key] += feature_stats.get(key, 0.0)
