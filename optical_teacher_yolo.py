...
class ConfigYOLO:
    ...
    FEATURE_FOREGROUND_TARGET = 0.70  # 光晕特征权重(前景目标)
    FEATURE_BACKGROUND_TARGET = 0.04

    FEATURE_HEATMAP_SIGMA = 0.18  # 光晕特征权重(高斯模糊)
    FEATURE_HEATMAP_POWER = 1.25  # 光晕特征权重(热图对比)
    FEATURE_BOX_FILL_VALUE = 0.04  # 光晕特征权重(框体填充)
    FEATURE_CORE_FILL_VALUE = 0.12  # 光晕特征权重(核心特征填充)
    FEATURE_CORE_RATIO = 0.32 # 光晕特征权重(小紫色框)

    FEATURE_FOREGROUND_BCE_MAX_GAIN = 6.0
    FEATURE_EDGE_BCE_GAIN = 2.5
    FEATURE_EDGE_MARGIN_RATIO = 0.06
    FEATURE_GAUSSIAN_TARGET_GAIN = 0.22 # 光晕特征权重(高斯模糊)

    FEATURE_TEXTURE_TARGET_BASE = 0.12  # 提高纹理基础值
    FEATURE_TEXTURE_TARGET_GAIN = 0.85  # 增强纹理增益
    FEATURE_TEXTURE_CONTRAST_WEIGHT = 0.75  # 增强纹理对比度
    FEATURE_TEXTURE_EDGE_WEIGHT = 0.65  # 增强纹理边缘权重
    FEATURE_TEXTURE_KEEP_QUANTILE = 0.65  # 保留纹理目标的前 65% 分位数
    FEATURE_TEXTURE_POWER = 1.80  # 增强纹理目标的对比度 增强纹理稀疏性
    FEATURE_TEXTURE_CENTER_BIAS = 0.62  # 增强纹理中心的对比度

    FEATURE_SMALL_OBJECT_AREA_REF = 32 * 32
    FEATURE_SMALL_OBJECT_MAX_GAIN = 2.4
    FEATURE_ACTIVE_TARGET_THRESH = 0.12
    DETECTOR_HEAT_AUX_WEIGHT = 0.13 #  0.18检测器热力图辅助损失权重
    DETECTOR_FEATURE_AUX_WEIGHT = 0.04 #  0.03检测器特征辅助损失权重
