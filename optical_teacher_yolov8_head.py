...
class C2f(nn.Module):
    """YOLOv8 C2f block adapted for the 1-channel optical teacher feature stream."""
    def __init__(self, in_channels, out_channels, num_blocks=2, shortcut=False, expansion=0.5):
        ...
        self.cv1 = ConvBNAct(in_channels, 2 * hidden, kernel_size=1)
        self.blocks = nn.ModuleList(Bottleneck(hidden, shortcut, expansion=1.0) for _ in range(num_blocks))
        self.cv2 = ConvBNAct((2 + num_blocks) * hidden, out_channels, kernel_size=1)
...
class YOLOv8DetectBranch(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        hidden = max(in_channels, 64)
        self.box = nn.Sequential(
            ConvBNAct(in_channels, hidden, kernel_size=3),
            ConvBNAct(hidden, hidden, kernel_size=3),
            nn.Conv2d(hidden, 3 * 4, kernel_size=1),
        )
        self.obj = nn.Sequential(
            ConvBNAct(in_channels, hidden, kernel_size=3),
            ConvBNAct(hidden, hidden, kernel_size=3),
            nn.Conv2d(hidden, 3 * 1, kernel_size=1),
        )
        cls_channels = out_channels - 3 * 5
        self.cls = nn.Sequential(
            ConvBNAct(in_channels, hidden, kernel_size=3),
            ConvBNAct(hidden, hidden, kernel_size=3),
            nn.Conv2d(hidden, cls_channels, kernel_size=1),
        )
...
class YOLOv8Head(nn.Module):
    def __init__(self, in_channels=1, out_channels=27, base_ch=32, c2f_blocks=2):
        super().__init__()
        if out_channels % 3 != 0 or out_channels < 18:
            raise ValueError(f"out_channels must be 3 * (5 + num_classes), got {out_channels}")

        self.stem = ConvBNAct(in_channels, base_ch, kernel_size=3)

        self.down1 = ConvBNAct(base_ch, base_ch * 2, kernel_size=3, stride=2)
        self.c2f1 = C2f(base_ch * 2, base_ch * 2, num_blocks=c2f_blocks, shortcut=True)
        self.down2 = ConvBNAct(base_ch * 2, base_ch * 4, kernel_size=3, stride=2)
        self.c2f2 = C2f(base_ch * 4, base_ch * 4, num_blocks=c2f_blocks, shortcut=True)
        self.down3 = ConvBNAct(base_ch * 4, base_ch * 8, kernel_size=3, stride=2)
        self.c2f3 = C2f(base_ch * 8, base_ch * 8, num_blocks=c2f_blocks, shortcut=True)
        self.down4 = ConvBNAct(base_ch * 8, base_ch * 8, kernel_size=3, stride=2)
        self.c2f4 = C2f(base_ch * 8, base_ch * 8, num_blocks=c2f_blocks, shortcut=True)
        self.down5 = ConvBNAct(base_ch * 8, base_ch * 8, kernel_size=3, stride=2)
        self.sppf = SPPF(base_ch * 8, base_ch * 8)

        self.up_p5 = nn.Upsample(scale_factor=2, mode="nearest")
        self.fuse_p4 = C2f(base_ch * 16, base_ch * 4, num_blocks=c2f_blocks, shortcut=False)
        self.up_p4 = nn.Upsample(scale_factor=2, mode="nearest")
        self.fuse_p3 = C2f(base_ch * 12, base_ch * 2, num_blocks=c2f_blocks, shortcut=False)

        self.down_p3 = ConvBNAct(base_ch * 2, base_ch * 4, kernel_size=3, stride=2)
        self.pan_p4 = C2f(base_ch * 8, base_ch * 4, num_blocks=c2f_blocks, shortcut=False)
        self.down_p4 = ConvBNAct(base_ch * 4, base_ch * 8, kernel_size=3, stride=2)
        self.pan_p5 = C2f(base_ch * 16, base_ch * 8, num_blocks=c2f_blocks, shortcut=False)

        self.head_p3 = YOLOv8DetectBranch(base_ch * 2, out_channels)
        self.head_p4 = YOLOv8DetectBranch(base_ch * 4, out_channels)
        self.head_p5 = YOLOv8DetectBranch(base_ch * 8, out_channels)
...
def train():
    ...
    log_to_file("Loading YOLOv8Head...")
